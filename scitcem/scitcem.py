...
def EMoptimize (A, D, thT_0, thN_0, 
    ...
    return {'p': p, 'logL': logL, 'thetaT': th['tumor'], 'thetaN': th['normal'], 'method': 'scitcem'}
...
def simulate_A (D, frac_tumor, thetaT, thetaN, seed=None):
    """get simulated matrix A given matrix D, tumor fraction, and parameters thetaT and thetaN"""
    if seed is not None:
        np.random.seed(seed)
...
def run_scitcem(indir, outdir, min_counts, thetaT, thetaN, use_vireo, estimate_power, nrep,
                frac_tumor, verbose):
    ...
    if take_SNPs.sum() > 0 and take_cells.sum() > 0 and data['A'][take_SNPs,:][:,take_cells].sum() > 0:
        ...
        if use_vireo:
            tmp = vireo_optimize(A, D, verbose)
        else:
            tmp = EMoptimize(A, D, thetaT, thetaN, verbose)

        df.loc[take_cells,'p'] = np.round(tmp['p'],3)

    else:

        sys.stderr.write("not enough SNP information in this sample!\n")

    df.to_csv(os.path.join(outdir, 'scitcem_results.txt'), sep='\t')
    pd.Series(tmp).to_csv(os.path.join(outdir, 'scitcem_parameters.txt'), sep='\t')
    ...
    if args.estimate_power:
        ...
        pd.DataFrame({'TPR': TPR, 'FPR': FPR}).to_csv(os.path.join(outdir,'scitcem_power_estimates.txt'),sep='\t')
